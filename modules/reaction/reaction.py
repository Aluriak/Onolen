...
class Onolen_Reaction(ircbot.SingleServerIRCBot):
        ...
        def __init__(self, Onolen, modeDebug):
                ...
                self.lastWordsUser = {} # clef = user; valeur = dernière phrase dite
        ...
        def on_pubmsg(self, server, ev):
                ...
                # appel à sed
                elif auteur != self.nom and REG_SED.search(message):
                        result = REG_SED.findall(message)
                        message = self.lastWordsUser[auteur] # récupération de la phrase
                        self.lastWordsUser[auteur] = self.lastWordsUser[auteur].replace(
                                result[0][0], result[0][1], 1)
                        print(auteur+" voulait dire : "+self.lastWordsUser[auteur])
                        self.sendMessage(auteur+" voulait dire : "+self.lastWordsUser[auteur])
                # Tout autre message, susceptible d'être corrigé plus tard
                elif auteur != self.nom:
                                self.lastWordsUser[auteur] = message
        ...
        def ajoutMessagerie(self, auteur, message):
                ...
                if dest == auteur or dest == self.nom: 
                        ...
                        if self.fiche.has_key(dest):
                                ...
                                self.sendMessage("Je dirais à "+dest+", de la part de "
                                                 +auteur+" que "+msg)
                        else:
